...
class User(Base):
    ...
    preferred_unit: Mapped[UnitEnum] = mapped_column(Enum(UnitEnum), default=UnitEnum.KM)

    bikes: Mapped[list["Bicycle"]] = relationship("Bicycle", back_populates="owner", cascade="all, delete-orphan")
...
class Bicycle(Base):
    ...
    rides: Mapped[list["Ride"]] = relationship("Ride", back_populates="bike", cascade="all, delete-orphan")
    components: Mapped[list["Component"]] = relationship("Component", back_populates="bike", cascade="all, delete-orphan")
...
class Ride(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True, index=True)
    bike_id: Mapped[int] = mapped_column(Integer, ForeignKey("bicycles.id"), nullable=False)
...
class Component(Base):
    ...
    service_records: Mapped[list["ServiceRecord"]] = relationship("ServiceRecord", back_populates="component", cascade="all, delete-orphan")
...
class ServiceRecord(Base):
    ...
    component: Mapped["Component"] = relationship("Component", back_populates="service_records")
